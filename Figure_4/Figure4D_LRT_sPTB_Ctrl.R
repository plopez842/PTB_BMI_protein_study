...
sub_sptb<-subset(merged_data_mod,Type %in% c("sptb"))
sub_merged_data<-rbind(sub_ctrl,sub_sptb)
...
dis_ctrl_g30<-all_comp_limma$sptbVsctrl_g30
dis_ctrl_g30<-dis_ctrl_g30[order(rownames(dis_ctrl_g30)),]
dis_ctrl_l30<-all_comp_limma$sptbVsctrl_l30
...
log2_comp_dis$protein=rownames(dis_ctrl_l30)
log2_comp_dis$log2FC_g30=dis_ctrl_g30$logFC
log2_comp_dis$pval_g30<-dis_ctrl_g30$P.Value
log2_comp_dis$log2FC_l30=dis_ctrl_l30$logFC
log2_comp_dis$pval_l30=dis_ctrl_l30$P.Value
...
log2_bmi_dis_plot<-ggplot(log2_comp_dis,aes(x=log2FC_l30,y=log2FC_g30,label=label))+
  ...
  ylim(-1.5,1.5)+
  xlim(-2,2)+
  scale_color_manual(values=c("yes"="#0073C2FF","no"="gray"))+
  
  geom_hline(yintercept=0,linetype="dashed",color="darkgray")+
  geom_vline(xintercept=0,linetype="dashed",color="darkgray")+
  theme_classic()+
  ggtitle("LRT of sPTB vs Ctrl",subtitle=paste0("Total DEP: ",sum(log2_comp_dis$padj_LRT_yes=="yes")))+
  ...
  xlab("log2FC(sPTB/Ctrl)\n<30 BMI kg/m3")+
  ylab("log2FC(sPTB/Ctrl)\n>=30 BMI kg/m3")
...
ggsave(log2_bmi_dis_plot,
       ...
       filename = "Fig4D_LRT_BMI_sPTB_vs_ctrl.pdf",
...
lapply(names(plot_ind_protein),
       function(x) ggsave(plot = plot_ind_protein[[x]],
                          filename = paste0("Fig4D_",subset(protein_apt_info, AptName %in% x)$Single_gene,".pdf"),
                          path = "../final_plots/figure4",
...
write.csv(df_protein_volcano,"../../Supplemental_data_revisions/fig4D_LRT_sPTB_ctrl.csv")
...
write.csv(univeriate_df,"../../Supplemental_data_revisions/fig4d_sPTB_univeriates.csv")

# Save RDS (for PCN)----
saveRDS(log2_comp_dis,file="../output_RDS_objs/fig4d_LRT_results_sPTB_Ctrl.rds")
