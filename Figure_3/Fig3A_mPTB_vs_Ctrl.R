...
y_var<-factor(y_var,levels=c("ctrl","mptb"))
...
my_colors=list(Status=c("ctrl"="#868686FF","mptb"="#CD534CFF"))
...
opts_sel <- list(n_trials = 1000, threshold = 0.75, return_count = TRUE,alpha=alpha_tuned)
...
nonzero_sel_features<-rownames(sel_features_count)[sel_features_count[,1]>750]
...
row_ha = HeatmapAnnotation(LV1 = new_scaled_df$LV1_score,
                           ...
                           col = list(Status=c("mptb"="#CD534CFF","ctrl"="#868686FF"),
                                      LV1 = y_col),
...
## Fig3A PLSDA
ggsave(filename = "Fig3A_PLSDA_mPTB_biplot.pdf",plt_scores_sel,units="in",
       path="../final_plots/figure3",
       width=4,
       height=4)


ggsave(filename = "Fig3A_PLSDA_mPTB_VIP_plot.pdf",plt_loadings_bar,units="in",
       path="../final_plots/figure3",
       width=4,
       height=2.5)

## Fig3A Heatmap
pdf(file="../final_plots/figure3/Fig3A_LV1_mPTB_proteins_heatmap",height=5,width=4.8)
draw(temp_ht, heatmap_legend_side = "bottom",
     annotation_legend_side="top")
dev.off()

#supp figures
ggsave(filename = "Sup_Fig3A_mPTB_LV1_loadings.pdf",plt_VIP_temp,units="in",
       path="../final_plots/Sup_figure3",
       width=4,
       height=2.5)

ggsave(filename = "Sup_Fig3C_mPTB_top_model_val.pdf",plt_val_p2,units="in",
       path="../final_plots/Sup_figure3",
       width=4,
       height=4)
...
# Save CSV----
## PLSDA Plot----
fig3a_left = data.frame(Patient_ID = paste0("Patient_",1:nrow(model_sel@scoreMN)),
                        Group = as.vector(y_var),
                        LV1 = as.vector(model_sel@scoreMN[,1]),
                        LV2 = as.vector(model_sel@scoreMN[,2]))


write.csv(fig3a_left,"../../Supplemental_data_revisions/fig3a_plsda_mptb_ctrl.csv")
...
write.csv(df_heatmap,"../../Supplemental_data_revisions/fig3a_heatmap_right.csv")


# Save RDS----
saveRDS(corr_mat_spear,file="../output_RDS_objs/fig3_PLSDA_mPTB_Ctrl.rds")
